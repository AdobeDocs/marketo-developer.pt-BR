...
title: getChannels
feature: SOAP
description: Saiba como o Marketo SOAP getChannels recupera status de progressão de canal, números de etapa e sinalizadores de sucesso, com exemplos de XML de solicitação e resposta.
...
Esta função recupera as seguintes informações sobre um canal configurado no Marketo:

- Status da progressão
- Número da etapa de cada status de progressão
- Se um status de progressão específico foi definido como sucesso

Você pode obter informações sobre todos os canais ou especificar uma lista específica de canais

## Solicitação

| Nome do campo | Obrigatório/Opcional | Descrição |
| --- | --- | --- |
| tag->values->stringItem | Opcional | Lista de canais que você deseja consultar. Se `<tag>` for omitido, você receberá informações sobre todos os canais |
