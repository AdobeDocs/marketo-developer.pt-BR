...
title: getTags
feature: SOAP, Tags
description: O Marketo SOAP getTags recupera tipos e valores de tag que não são de canal com filtros de tag opcionais, além de campos de solicitação e amostras completas de resposta de solicitação XML.
...
Essa função recupera as seguintes informações sobre tags que não são de canal configuradas no Marketo.

- Tipo de marca
- Valor da tag

Você pode obter informações sobre todas as tags que não sejam de canal ou especificar uma lista específica de tags e/ou valores.

## Solicitação

| Nome do campo | Obrigatório/Opcional | Descrição |
| --- | --- | --- |
| tagList->tag | Opcional | Lista de tags que você deseja consultar. Cada tag pode ter um tipo e uma matriz de valores como strings. Se `<tagList>` for omitido, você receberá informações sobre todos os canais |
