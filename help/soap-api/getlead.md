...
title: getLead
feature: SOAP
description: Use o Marketo SOAP getLead para obter um único lead e todos os valores de campo por IDNUM, email, cookie ou SFDC IDs, com amostra de solicitação e resposta XML mais PHP.
...
Esta função recupera um único registro de cliente potencial do Marketo, com todos os valores de campo dos campos incorporados e personalizados, para um cliente potencial identificado pela chave fornecida (LeadKey). Se o lead existir com base nos parâmetros de entrada, os atributos do registro de lead serão retornados no resultado.

OBSERVAÇÃO: os atributos de cliente potencial que são do tipo de dados de cadeia de caracteres e estão vazios não serão retornados como parte da resposta.
