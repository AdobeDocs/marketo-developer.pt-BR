...
feature: REST API
description: "Extração em lote de dados de clientes potenciais."
...
Os clientes em potencial são compatíveis com várias opções de filtro. Determinados filtros, incluindo o `updatedAt`, `smartListName`, e `smartListId` exigir componentes de infraestrutura adicionais que ainda não foram lançados para todas as assinaturas. Somente um tipo de filtro pode ser especificado por trabalho de exportação.
...
|---|---|---|---|
| campos | Matriz[String] | Sim | O parâmetro fields aceita uma matriz JSON de cadeias de caracteres. Cada string deve ser o nome da API REST de um campo de lead Marketo. Os campos listados são incluídos no arquivo exportado. O cabeçalho de coluna para cada campo será o nome da API REST de cada campo, a menos que seja substituído por columnHeader. Observação: quando a variável [!DNL Adobe Experience Cloud Audience Sharing] estiver ativado, ocorrerá um processo de sincronização de cookies que associa [!DNL Adobe Experience Cloud] ID (ECID) com clientes potenciais da Marketo. Você pode especificar o campo &quot;ecids&quot; para incluir ECIDs no arquivo de exportação. |
...
Os parâmetros do processo são definidos antes do início da exportação usando o [Criar tarefa de exportação de clientes em potencial](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/createExportLeadsUsingPOST) terminal. Precisamos definir o `fields` que são necessários para a exportação, o tipo de parâmetros do `filter`, o `format` do arquivo e os nomes do cabeçalho da coluna, se houver.
...
Essa solicitação começará a exportar um conjunto de clientes potenciais criados entre 1º de janeiro de 2017 e 31 de janeiro de 2017, incluindo os valores dos segmentos correspondentes `firstName`, `lastName`, `id`, e `email` campos.
...
Isso retorna uma resposta de status indicando que o processo foi criado. A tarefa foi definida e criada, mas ainda não foi iniciada. Para isso, a [Enfileirar tarefa de lead de exportação](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/enqueueExportLeadsUsingPOST) O endpoint deve ser chamado usando o exportId da resposta de status de criação:
...
Isso responde com uma `status` de &quot;Em fila&quot;, após o que será definido como &quot;Processando&quot; quando houver um slot de exportação disponível.

## Status do trabalho de pesquisa

`Note:` O status só pode ser recuperado para trabalhos criados pelo mesmo usuário da API.

Como esse é um endpoint assíncrono, depois de criar o trabalho, devemos pesquisar seu status para determinar seu progresso. Consultar usando o [Obter Status do Trabalho de Exportação de Cliente Potencial](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/getExportLeadsStatusUsingGET) terminal. O status só é atualizado uma vez a cada 60 segundos, portanto, uma frequência de polling inferior a essa não é recomendada e, em quase todos os casos, ainda é excessiva. Vamos dar uma olhada na pesquisa.
...
Para recuperar o arquivo de uma exportação de clientes potenciais concluída, basta chamar o [Obter Arquivo de Cliente Potencial de Exportação](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/getExportLeadsFileUsingGET) terminal com seu `exportId`.
...
Se um campo de cliente em potencial solicitado estiver vazio (não contiver dados), `null` é colocado no campo correspondente no arquivo de exportação. No exemplo abaixo, o campo de email do lead retornado está vazio.
...
Para oferecer suporte à recuperação parcial e de fácil retomada dos dados extraídos, o endpoint do arquivo oferece suporte opcional ao Intervalo do cabeçalho HTTP do tipo bytes. Se o cabeçalho não estiver definido, todo o conteúdo será retornado. Leia mais sobre como usar o cabeçalho Intervalo com o Marketo [Extração em massa](bulk-extract.md).

## Cancelar um trabalho

Se uma tarefa for configurada incorretamente ou se se tornar desnecessária, ela poderá ser facilmente cancelada usando o [Cancelar tarefa de exportar lead](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Bulk-Export-Leads/operation/cancelExportLeadsUsingPOST) endpoint:
