...
O padrão para [consultar oportunidades](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Opportunities/operation/getOpportunitiesUsingGET) acompanha de perto a da API de clientes potenciais, com a restrição adicional de `filterType` aceita os campos listados na variável `searchableFields` ou da chamada descrever correspondente, ou dedupeFields.  Observe que se estiver usando campos de oportunidade personalizados, somente os campos de oportunidade personalizados do tipo String ou Integer serão listados na matriz searchableFields.
...
Você também pode incluir os parâmetros de consulta opcionais `fields`, para retornar campos de oportunidade adicionais, `nextPageToken`, para paginação em conjuntos maiores que o tamanho do lote, `batchSize`, que assume como padrão e tem um máximo de 300.  Ao solicitar uma lista de `fields`, se um campo específico for solicitado, mas não for retornado, o valor será considerado nulo.
...
A API responderá com a `marketoGUID` para cada registro, bem como uma `status` campo, indicando o sucesso ou falha individual de cada registro, e um `seq` campo usado para correlacionar os registros enviados à ordem da resposta.  O número no campo é o índice do registro enviado na solicitação.
...
A variável [Obter campo de oportunidade por nome](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Opportunities/operation/getOpportunityFieldByNameUsingGET) O endpoint recupera metadados de um único campo no objeto da empresa.  O necessário `fieldApiName` parâmetro de caminho especifica o nome da API do campo.  A resposta é como o endpoint Descrever oportunidade, mas contém metadados adicionais, como o `isCustom` atributo que indica se o campo é um campo personalizado.
...
Você pode excluir oportunidades por campos de desduplicação ou campo de id. Especificar usando o `deleteBy` parâmetro com um valor dedupeFields ou idField. Se não especificado, o padrão é dedupeFields. O corpo da solicitação contém um `input` matriz de oportunidades a serem excluídas. São permitidas no máximo 300 oportunidades por chamada.
