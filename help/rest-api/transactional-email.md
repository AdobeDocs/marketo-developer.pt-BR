...
feature: REST API
description: "Lidar com emails transacionais para campanhas de Solicitação."
...
Um caso de uso comum para a API do Marketo é acionar o envio de emails transacionais para registros específicos por meio do [Solicitar campanha](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Campaigns/operation/triggerCampaignUsingPOST) chamada à API. Há alguns requisitos de configuração no Marketo para executar a chamada necessária com a API REST do Marketo.

- O recipient deve ter um registro no Marketo
- Deve haver um email transacional criado e aprovado na instância do Marketo.
- Deve haver uma campanha de acionador ativa com a mensagem &quot;A campanha é solicitada, 1. Source: Web Service API&quot;, que é configurada para enviar o email

Primeiro [criar e aprovar seu email](https://experienceleague.adobe.com/docs/marketo/using/home.html?lang=pt-BR). Se o email for realmente transacional, você provavelmente precisará configurá-lo como operacional, mas certifique-se de que ele se qualifique legalmente como operacional. Isso é configurado no com a tela Editar em Ações de email > Configurações de email:
...
Se você nunca criou campanhas, confira o [Criar uma nova campanha inteligente](https://experienceleague.adobe.com/docs/marketo/using/product-docs/core-marketo-concepts/smart-campaigns/creating-a-smart-campaign/create-a-new-smart-campaign.html) artigo. Depois de criar sua campanha, devemos seguir essas etapas. Configure sua Smart List com o acionador Campanha solicitada:
...
**Nota:** Nos exemplos de Java abaixo, estamos usando o [pacote minimal-json](https://github.com/ralfstx/minimal-json) para lidar com representações JSON em nosso código.

A primeira parte do envio de um email transacional por meio da API é garantir que um registro com o endereço de email correspondente exista na instância do Marketo e que tenhamos acesso à sua ID de lead. Para os fins desta publicação, pressupomos que os endereços de email já estejam no Marketo e devemos recuperar apenas a ID do registro. Para isso, estamos usando o [Obter clientes em potencial por tipo de filtro](https://developer.adobe.com/marketo-apis/api/mapi/#tag/Leads/operation/getLeadsByFilterUsingGET) chame. Vamos examinar nosso Método principal para solicitar a campanha:
